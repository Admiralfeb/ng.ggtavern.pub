...
/**
 * Works with the Calendar, generating, gets data from database, etc.
 */
@Injectable()
export class CalendarService {
  ...
  /**
   * Gets the calendar from the database
   */
  async getCalendar(): Promise<Calendar> {
    ...
    return { id, name, startDay: start, calendarEvents: events };
  }
  ...
  /**
   * Generates the Calendar based upon the month information provided.
   * @param monthConfig Month information
   */
  private generateCalendar(monthConfig: Month): CalendarEvent[] {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (!event) {
        event = { id: '0', year, month, day: date, eventName: null, imgString: null, time: null };
      }
